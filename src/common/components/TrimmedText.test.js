...
describe('TrimmedText', () => {
  test('render normal text if length is less than or equal to max size', () => {
    [true, false].forEach(allowShowMore => {
      const wrapper = shallow(
        <TrimmedText
          {...getDefaultTrimmedTextProps({ allowShowMore: allowShowMore })}
        />
      );
  ...
  test('render trimmed text if length is greater than max size', () => {
    const wrapper = shallow(
      <TrimmedText {...getDefaultTrimmedTextProps({ maxSize: 5 })} />
    );
    expect(wrapper.text()).toEqual('01234...');
    expect(wrapper.find('[data-test-id="trimmed-text-button"]').length).toEqual(
      0
    );
  });
  ...
  test('render show more button if configured', () => {
    const wrapper = shallow(
      <TrimmedText
        {...getDefaultTrimmedTextProps({ maxSize: 5, allowShowMore: true })}
      />
    );
    expect(wrapper.find('[data-test-id="trimmed-text-button"]').length).toEqual(
      1
    ...
    expect(
      wrapper.find('[data-test-id="trimmed-text-button"]').children(0).text()
    ).toEqual('expand');

...
test('Setting key-value pairs in one scope does not affect the other', () => {
  ...
  const store1 = LocalStorageUtils.getStoreForComponent(
    'AnotherTestComponent',
    1
  );
  const store2 = LocalStorageUtils.getStoreForComponent('SomeTestComponent', 2);
  const persistedState0 = new ExperimentPagePersistedState({
    searchInput: 'params.ollKorrect'
  });
  const persistedState1 = new ExperimentPagePersistedState({
    searchInput: 'metrics.ok'
  });
  [store1, store2].forEach(otherStore => {
    ...
    expect(store0.loadComponentState().searchInput).toEqual(
      'params.ollKorrect'
    );
...
test('Overwriting key-value pairs is possible', () => {
  ...
  store.saveComponentState(
    new ExperimentPagePersistedState({ searchInput: 'params.ollKorrect' })
  );
  expect(store.loadComponentState().searchInput).toEqual('params.ollKorrect');
  store.saveComponentState(
    new ExperimentPagePersistedState({ searchInput: 'params.okay' })
  );
  expect(store.loadComponentState().searchInput).toEqual('params.okay');
});

test('Session scoped storage works', () => {
  const store = LocalStorageUtils.getSessionScopedStoreForComponent(
    'SomeTestComponent',
    1
  );
  store.setItem('a', 'b');
  ...
  const store1 = LocalStorageUtils.getSessionScopedStoreForComponent(
    'AnotherTestComponent',
    1
  );
  const store2 = LocalStorageUtils.getSessionScopedStoreForComponent(
    'SomeTestComponent',
    2
  );
  const persistedState0 = new ExperimentPagePersistedState({

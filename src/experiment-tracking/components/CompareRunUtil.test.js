...
describe('CompareRunUtil', () => {
  const makeSublist = obj => {
    ...
    Object.entries(obj).forEach(
      ([key, value]) =>
        (newObj[key] = Metric.fromJs({ key: key, value: value }))
    );
  ...
  test('findInList - finds various value types in list', () => {
    ...
    expect(CompareRunUtil.findInList(list[0], 'timestamp').value).toEqual(
      1571684492701
    );
    expect(CompareRunUtil.findInList(list[1], 'timestamp').value).toEqual(
      1571684492702
    );

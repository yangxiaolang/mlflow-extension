...
export class CompareRunView extends Component {
  ...
  constructor(props) {
    ...
    this.onCompareRunTableScrollHandler =
      this.onCompareRunTableScrollHandler.bind(this);
  ...
  onCompareRunTableScrollHandler(e) {
    const blocks =
      this.compareRunViewRef.current.querySelectorAll('.compare-run-table');
  ...
  getTableColumnWidth() {
    ...
    if (this.state.tableWidth !== null) {
      colWidth = Math.round(
        this.state.tableWidth / (this.props.runInfos.length + 1)
      );
  ...
  shouldShowExperimentNameRow() {
    return (
      this.props.hasComparedExperimentsBefore || this.hasMultipleExperiments()
    );
  }
  ...
  renderTimeRows(colWidthStyle) {
    ...
    const getTimeAttributes = runInfo => {
      ...
      return {
        ...
        duration:
          startTime && endTime ? Utils.getDuration(startTime, endTime) : unknown
      };
    };
    ...
    const rows = [
      {
        ...
        data: timeAttributes.map(({ runUuid, startTime }) => [
          runUuid,
          startTime
        ])
      },
  ...
  render() {
    ...
    const { runInfos, runNames, paramLists, metricLists, runUuids } =
      this.props;
      ...
      <div className="CompareRunView" ref={this.compareRunViewRef}>
        ...
        >
          <Switch
            ...
            onChange={(checked, e) =>
              this.setState({ onlyShowParamDiff: checked })
            }
          />
        ...
        <CollapsibleSection
          title={this.props.intl.formatMessage({
            ...
            description:
              'Row group title for metrics of runs on the experiment compare runs page'
          })}
        >
          <Switch
            checked={this.state.onlyShowMetricDiff}
            onChange={(checked, e) =>
              this.setState({ onlyShowMetricDiff: checked })
            }
          />
          <Typography.Text hint color="secondary">
            <FormattedMessage
        ...
        >
          <Switch
            ...
            onChange={(checked, e) =>
              this.setState({ onlyShowTagDiff: checked })
            }
          />
          <Typography.Text hint color="secondary">
            <FormattedMessage
  ...
  renderDataRows(
    ...
    keys.forEach(
      k => (data[k] = { values: Array(list.length).fill(undefined) })
    );
...
const mapStateToProps = (state, ownProps) => {
  ...
  const experiments = experimentIds.map(experimentId =>
    getExperiment(experimentId, state)
  );
  runUuids.forEach(runUuid => {
    ...
    const visibleTags = Utils.getVisibleTagValues(runTags).map(
      ([key, value]) => ({
        key,
        value
      })

...
export const updateRegisteredModelApi = (
  name,
  description,
  id = getUUID()
) => ({
...
export const deleteRegisteredModelApi = (
  model,
  id = getUUID(),
  localUpdateOnly
) => ({
...
export const setRegisteredModelTagApi = (
  modelName,
  key,
  value,
  id = getUUID()
...
export const deleteRegisteredModelTagApi = (
  modelName,
  key,
  id = getUUID()
) => ({
...
export const getModelVersionActivitiesApi = (
  modelName,
  version,
  id = getUUID()
) => ({
  ...
  const filter = Object.keys(filterObj)
    .map(key => {
      if (Array.isArray(filterObj[key]) && filterObj[key].length > 1) {
        return `${key} IN (${filterObj[key]
          .map(elem => resolveFilterValue(elem))
          .join()})`;
...
export const deleteModelVersionApi = (
  modelName,
  version,
  id = getUUID(),
  localUpdateOnly
...
export const setModelVersionTagApi = (
  modelName,
  version,
  key,
  value,
...
export const deleteModelVersionTagApi = (
  modelName,
  version,
  key,
  id = getUUID()

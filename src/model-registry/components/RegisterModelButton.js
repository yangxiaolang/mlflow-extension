...
export class RegisterModelButtonImpl extends React.Component {
  ...
  handleSearchRegisteredModels = input => {
    this.props.searchRegisteredModelsApi(
      getModelNameFilter(input),
      MAX_SEARCH_REGISTERED_MODELS
    );
  };

  reloadModelVersionsForCurrentRun = () => {
    const { runUuid } = this.props;
    return this.props.searchModelVersionsApi(
      { run_id: runUuid },
      this.searchModelVersionRequestId
    );
  };
      ...
      <div className="register-model-btn-wrapper">
        ...
        <Modal
          title={this.props.intl.formatMessage({
            defaultMessage: 'Register Model',
            description:
              'Register model modal title to register the model for deployment'
          })}
          width={540}
          visible={visible}
        ...
        >
          <RegisterModelForm
            ...
            innerRef={this.form}
            onSearchRegisteredModels={_.debounce(
              this.handleSearchRegisteredModels,
              300
            )}
          />
        </Modal>

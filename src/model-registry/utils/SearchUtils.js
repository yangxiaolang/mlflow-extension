...
export function getModelNameFilter(query) {
  if (query) {
    return `${REGISTERED_MODELS_SEARCH_NAME_FIELD} ilike ${resolveFilterValue(
      query,
      true
    )}`;
  } else {
...
export function getCombinedSearchFilter({
  ...
  const filters = [];
  const initialFilter = query.includes('tags.')
    ? query
    : getModelNameFilter(query);
  if (initialFilter) {
...
export function constructSearchInputFromURLState(urlState) {
  ...
  if ('nameSearchInput' in urlState && 'tagSearchInput' in urlState) {
    return (
      getModelNameFilter(urlState['nameSearchInput']) +
      ' AND ' +
